...
The most important thing that you need to keep in mind is htat if you don't specify a `WHERE` clause, all of the entries inside the `users` table would be updated and all users would have the `username` set to `bobbyiliev`.

> Important: You need to be careful when you use the `UPDATE` statement without a `WHERE` caluse as every single row will be updated.

If you have been following along all of the user entries in our `users` table currtly have no data in the `about` column:
...
Let's now say that we wanted to update the `about` column for the user with id of 2. In this case we need to specify a `WHERE` clause followed by the ID of the user that we want to update as follows:
...
Now if you again run the `SELECT id,username,about FROM users` query you would see that user with `id` now has an updated `about` column data:
